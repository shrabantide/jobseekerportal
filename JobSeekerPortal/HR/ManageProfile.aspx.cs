...
namespace JobSeekerPortal.HR
{
    public partial class ManageProfile : System.Web.UI.Page
    {
        ...
        int userid;
        string fn, ln, cname, contact, gen, addr;
        ...
        protected void FetchProfile()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    gen = "";
                    addr = "";
                    Response.Write("<script>alert('Kindly add profile else the vacancy details may not be displayed to the employees.')</script>");
                }
                txtfname.Text = fn;
                txtlname.Text = ln;
                txtcname.Text = cname;
        ...
        protected void btnprofile_Click(object sender, EventArgs e)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                try
                {
                    ...
                    string update = "update tblhr set fname = @fname, lname = @lname, companyname = @companyname, contact = @contactno, gender = @gen, address = @add where emailid = @emailid";
                    SqlCommand cmd = new SqlCommand(update, con);
                    cmd.Parameters.AddWithValue("@fname", txtfname.Text.ToString());
                    cmd.Parameters.AddWithValue("@lname", txtlname.Text.ToString());
                    cmd.Parameters.AddWithValue("@companyname", txtcname.Text.ToString());
            ...
            else 
            {
                try
                {
                    ...
                    string update = "insert into tblhr values (@fname, @lname,@companyname, @contactno, @gen, @add, @emailid)";
                    SqlCommand cmd = new SqlCommand(update, con);
                    cmd.Parameters.AddWithValue("@fname", txtfname.Text.ToString());
                    cmd.Parameters.AddWithValue("@lname", txtlname.Text.ToString());
                    cmd.Parameters.AddWithValue("@companyname", txtcname.Text.ToString());
